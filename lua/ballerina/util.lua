...
-- Resolve the `bal` binary: config override, then PATH lookup, then the
-- known install locations. Returns nil when nothing is found; "not found"
-- is never cached, so an install done mid-session is picked up.
---@return string|nil
M.bal_cmd = function()
